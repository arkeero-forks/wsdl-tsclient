...
export class ParsedWsdl {
  ...
  findDefinition(definitionName: string): Definition {
    return this.definitions.find(def => def.name === definitionName) as Definition;
  }
  ...
  findNonCollisionDefinitionName(defName: string): string {
    ...
    if (
      !this.definitions.find(def => isInSensitive
        ? def.name.toLowerCase() === defNameToCheck
        : def.name === defNameToCheck)
    ) {
