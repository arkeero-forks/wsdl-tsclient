...
import { Definition, Method, ParsedWsdl } from "./models/parsed-wsdl.js";
 ...
 */
function addSafeImport(imports: OptionalKind<ImportDeclarationStructure>[],
...
function createProperty(name: string,
  type: string,
  doc: string,
  isArray: boolean,
  optional = true): PropertySignatureStructure {
  return {
...
function generateDefinitionFile(project: Project,
  definition: null | Definition,
  defDir: string,
  stack: string[],
  generated: Definition[],
  options: GeneratorOptions): void {
  ...
  for (const prop of definition!.properties) {
    ...
    if (prop.kind === "PRIMITIVE") {
      ...
      if (!generated.includes(prop.ref)) {
        ...
        generateDefinitionFile(project, prop.ref, defDir, [
          ... stack,
          prop.ref.name
        ], generated, options);
      }
...
export async function generate(parsedWsdl: ParsedWsdl,
  outDir: string,
  options: Partial<GeneratorOptions>): Promise<void> {
  ...
  for (const service of parsedWsdl.services) {
      ...
      for (const method of port.methods) {
        ...
        if (method.paramDefinition !== null) {
          if (!allDefinitions.includes(method.paramDefinition)) {
            ...
            addSafeImport(clientImports, `./definitions/${method.paramDefinition.name}`, method.paramDefinition.name, mergedOptions);
          }
          addSafeImport(portImports, `../definitions/${method.paramDefinition.name}`, method.paramDefinition.name, mergedOptions);
        }
        if (method.returnDefinition !== null) {
          if (!allDefinitions.includes(method.returnDefinition)) {
            ...
            addSafeImport(clientImports, `./definitions/${method.returnDefinition.name}`, method.returnDefinition.name, mergedOptions);
          }
          addSafeImport(portImports, `../definitions/${method.returnDefinition.name}`, method.returnDefinition.name, mergedOptions);
        }
    ...
    if (!mergedOptions.emitDefinitionsOnly) {
      ...
      clientServices.push({
        "name": sanitizePropName(service.name),
        "type": service.name
      });
  ...
  indexFile.addExportDeclarations(allDefinitions.map(def => ({
    "namedExports": [ def.name ],
    "moduleSpecifier": `./definitions/${def.name}${mergedOptions.exportAsModule
      ? ".js"
      : ""}`
  })));
  if (!mergedOptions.emitDefinitionsOnly) {
    ...
    indexFile.addExportDeclarations([
      {
        "namedExports": [
          "createClientAsync",
          `${parsedWsdl.name}Client`
        ],
        "moduleSpecifier": `./client${mergedOptions.exportAsModule
          ? ".js"
          : ""}`
      }
    ]);
    indexFile.addExportDeclarations(parsedWsdl.services.map(service => ({
      ...
      "moduleSpecifier": `./services/${service.name}${mergedOptions.exportAsModule
        ? ".js"
        : ""}`
    })));
    indexFile.addExportDeclarations(parsedWsdl.ports.map(port => ({
      "namedExports": [ port.name ],
      "moduleSpecifier": `./ports/${port.name}${mergedOptions.exportAsModule
        ? ".js"
        : ""}`
    })));
  }
