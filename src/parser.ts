...
import { ComplexTypeElement } from "soap/lib/wsdl/elements.js";
import { open_wsdl } from "soap/lib/wsdl/index.js";
import {
  Definition, Method, ParsedWsdl, Port, Service
} from "./models/parsed-wsdl.js";
 ...
 */
function parseDefinition(parsedWsdl: ParsedWsdl,
  options: ParserOptions,
  name: string,
  defParts: { [propNameType: string]: any },
  stack: string[],
  visitedDefs: Array<VisitedDefinition>): Definition {
  ...
  const definition: Definition = {
    "name": `${options.modelNamePreffix}${changeCase(nonCollisionDefName, {
      "pascalCase": true
    })}${
  ...
  visitedDefs.push({
    ...
    "parts": defParts,
    definition
  }); // NOTE: cache reference to this defintion globally (for avoiding circular references)
  if (defParts) {
    ...
    if ("undefined" in defParts && defParts.undefined === undefined) {
      ...
      Object.entries(defParts).forEach(([
        propName,
        type
      ]) => {
        if (propName === "targetNSAlias") {
          ...
          if (typeof type === "string") {
            ...
            if (visited) {
              ...
              try {
                const subDefinition = parseDefinition(parsedWsdl, options, stripedPropName, type, [
                  ... stack,
                  propName
                ], visitedDefs);
          ...
          if (typeof type === "string") {
            ...
            if (reference) {
              ...
              try {
                const subDefinition = parseDefinition(parsedWsdl, options, propName, type, [
                  ... stack,
                  propName
                ], visitedDefs);
  ...
  return new Promise((resolve, reject) => {
    open_wsdl(wsdlPath, {
      ...
      for (const [
        serviceName,
        service
      ] of Object.entries(wsdl.definitions.services)) {
        ...
        for (const [
          portName,
          port
        ] of Object.entries(service.ports)) {
          ...
          for (const [
            methodName,
            method
          ] of Object.entries(port.binding.methods)) {
            ...
            let inputDefinition: Definition; // default type
            ...
            let outputDefinition: Definition; // default type, `{}` or `unknown` ?
